...
pub trait Node: Send + Sync {
    ...
    fn prep(&self, store: &SharedStore) -> impl Future<Output = Result<Self::PrepRes>> + Send;

    /// Execute the core operation
    fn exec(&self, prep_res: Self::PrepRes) -> impl Future<Output = Result<Self::ExecRes>> + Send;
    ...
    fn run(&self, store: &mut SharedStore) -> impl Future<Output = Result<Self::ExecRes>> + Send {
        async {
